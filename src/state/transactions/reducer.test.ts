...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.MAINNET_BSC]).toBeTruthy()
      expect(txs[ChainId.MAINNET_BSC]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.MAINNET_BSC]?.['0x0']
  ...
  describe('finalizeTransaction', () => {
    ...
    it('sets receipt', () => {
      ...
      const tx = store.getState()[ChainId.TESTNET_BSC]?.['0x0']
      expect(tx?.summary).toEqual('hello world')
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.MAINNET_BSC), String(ChainId.TESTNET_BSC)])
      expect(Object.keys(store.getState()[ChainId.MAINNET_BSC] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.TESTNET_BSC] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.MAINNET_BSC }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.MAINNET_BSC), String(ChainId.TESTNET_BSC)])
      expect(Object.keys(store.getState()[ChainId.MAINNET_BSC] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.TESTNET_BSC] ?? {})).toEqual(['0x1'])
    })
  })
})
