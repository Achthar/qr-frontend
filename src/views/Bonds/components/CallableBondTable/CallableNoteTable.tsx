...
import { useClosedCallableMarkets } from 'state/bonds/hooks'
...
const CallNoteRow: React.FC<CallableNoteProps> = ({ isLast, isFirst, note, userDataReady, isMobile, reqPrice }) => {
  ...
  const config = useMemo(() => {
    try {
      return getConfigForVanillaNote(chainId, note, closed, cfg)
    } catch (error) {
      console.log(error)
      return null
    }
...
function compareMaturities(a: CallableNote, b: CallableNote) {
